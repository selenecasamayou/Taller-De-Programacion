...
type 
  ...
  procedure ImprimirVector (v: vector; dimL: rango3);   // punto B
  var
     i: integer;
  begin
     write ('         -');
  ...
  procedure Ordenar (var v: vector; dimL: rango3);   // punto C - seleccion
  var 
  ...
  procedure Eliminar (var v: vector; var dimL: rango3; valorInferior, valorSuperior: rango1);  // punto E
  ...
  Begin
     ...
     if (posInferior <> 0) then begin
         ...
         if (posSuperior > posInferior) then begin
           salto:= posInferior - posSuperior + 1;
           for i:= posInferior to diml do
             v[i]:= v[i + salto];
            diml:= diml - 1;
  ...
  procedure GenerarLista (v: vector; dimL: rango3; var L: lista);     // punto G


    procedure AgregarAtras (var L, ult: lista; elem: elemento);         // punto G
    var
       nue:lista;
    begin
       new(nue);
  ...
  procedure ImprimirLista (L: lista);      // punto H
  begin
